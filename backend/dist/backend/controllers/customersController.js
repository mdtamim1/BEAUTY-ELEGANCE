...
export const registerCustomer = (req, res) => {
    ...
    db.get('SELECT id FROM customers WHERE email = ?', [email], (err, row) => {
        ...
        bcrypt.hash(password, 10, (err, hash) => {
            ...
            db.run(`INSERT INTO customers (id, first_name, last_name, email, password_hash, phone)
         VALUES (?, ?, ?, ?, ?, ?)`, [customerId, first_name, last_name, email, hash, phone || ''], function (err) {
                if (err) {
                    console.error('Error creating customer:', err);
                    return res.status(500).json({ status: 'error', message: 'Failed to create customer' });
                }
                // Return JWT
                const token = jwt.sign({ id: customerId, email, role: 'customer', name }, JWT_SECRET, { expiresIn: '30d' });
                res.json({
                    status: 'success',
                    data: {
                        token,
...
export const loginCustomer = (req, res) => {
    ...
    db.get('SELECT * FROM customers WHERE email = ?', [email], (err, customer) => {
        ...
        bcrypt.compare(password, customer.password_hash, (err, isMatch) => {
            ...
            db.all('SELECT * FROM customer_addresses WHERE customer_id = ? ORDER BY is_default DESC, created_at DESC', [customer.id], (err, rows) => {
                ...
                const token = jwt.sign({ id: customer.id, email: customer.email, role: 'customer', name: fullName }, JWT_SECRET, { expiresIn: '30d' });
                res.json({
                    status: 'success',
                    data: {
                        token,
...
export const loginGmailCustomer = async (req, res) => {
    ...
    try {
        ...
        db.get('SELECT * FROM customers WHERE email = ?', [email], (err, existing) => {
            if (err) {
            ...
            if (existing) {
                // Fetch addresses
                db.all('SELECT * FROM customer_addresses WHERE customer_id = ? ORDER BY is_default DESC, created_at DESC', [existing.id], (err, rows) => {
                    const addresses = (rows || []).map(r => ({
                        id: r.id,
                        label: r.label,
                        name: r.name,
                    ...
                    const token = jwt.sign({ id: existing.id, email: existing.email, role: 'customer', name: fullName }, JWT_SECRET, { expiresIn: '30d' });
                    res.json({
                        status: 'success',
                        data: {
                            token,
                ...
                db.run(`INSERT INTO customers (id, first_name, last_name, email, password_hash, phone)
           VALUES (?, ?, ?, ?, ?, ?)`, [customerId, first_name, last_name, email, dummyHash, ''], function (err) {
                    if (err) {
...
export const getCustomerProfile = (req, res) => {
    ...
    db.get('SELECT * FROM customers WHERE id = ?', [customerId], (err, customer) => {
        ...
        db.all('SELECT * FROM customer_addresses WHERE customer_id = ? ORDER BY is_default DESC, created_at DESC', [customerId], (err, rows) => {
            const addresses = (rows || []).map(r => ({
                id: r.id,
                label: r.label,
                name: r.name,
...
export const updateCustomerProfile = (req, res) => {
    ...
    db.run(`UPDATE customers 
     ...
     WHERE id = ?`, [first_name, last_name, phone || '', address || '', customerId], function (err) {
        if (err) {
...
export const addAddress = (req, res) => {
    ...
    db.get('SELECT COUNT(*) as count FROM customer_addresses WHERE customer_id = ?', [customerId], (err, row) => {
        const isFirst = !err && row && row.count === 0;
        const shouldBeDefault = isFirst ? 1 : (isDefault ? 1 : 0);
        const insertAddress = () => {
            db.run(`INSERT INTO customer_addresses (id, customer_id, label, name, phone, address, is_default)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [addressId, customerId, label, name, phone, address, shouldBeDefault], function (err) {
                if (err) {
...
export const updateAddress = (req, res) => {
    ...
    const { label, name, phone, address, isDefault } = req.body;
    const runUpdate = () => {
        db.run(`UPDATE customer_addresses 
       SET label = ?, name = ?, phone = ?, address = ?, is_default = ?
       WHERE id = ? AND customer_id = ?`, [label, name, phone, address, isDefault ? 1 : 0, addressId, customerId], function (err) {
            if (err) {
...
export const deleteAddress = (req, res) => {
    ...
    db.get('SELECT is_default FROM customer_addresses WHERE id = ? AND customer_id = ?', [addressId, customerId], (err, row) => {
        if (err || !row) {
            return res.status(444).json({ status: 'error', message: 'Address not found' });
        }
        const wasDefault = row.is_default === 1;
        db.run('DELETE FROM customer_addresses WHERE id = ? AND customer_id = ?', [addressId, customerId], function (err) {
            if (err) {
                return res.status(500).json({ status: 'error', message: 'Database error' });
            }
            if (wasDefault) {
                // Find another address and make it default
                db.get('SELECT id FROM customer_addresses WHERE customer_id = ? LIMIT 1', [customerId], (err, another) => {
                    if (another) {
                        db.run('UPDATE customer_addresses SET is_default = 1 WHERE id = ?', [another.id]);
                    }
                });
            }
...
export const setDefaultAddress = (req, res) => {
    ...
    db.run('UPDATE customer_addresses SET is_default = 0 WHERE customer_id = ?', [customerId], (err) => {
        ...
        db.run('UPDATE customer_addresses SET is_default = 1 WHERE id = ? AND customer_id = ?', [addressId, customerId], function (err) {
            if (err) {
                return res.status(500).json({ status: 'error', message: 'Database error' });
            }
