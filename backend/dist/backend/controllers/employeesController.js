...
export const getEmployees = (req, res) => {
    db.all(`SELECT e.id, e.first_name, e.last_name, e.email, e.status, e.department, e.created_at, e.last_login_at, r.name as role, r.id as role_id 
     FROM employees e 
...
export const updateEmployee = (req, res) => {
    ...
    db.run(`UPDATE employees 
     ...
     WHERE id = ?`, [role_id, status, department || 'Operations', employeeId], function (err) {
        if (err) {
...
export const getInvitations = (req, res) => {
    db.all(`SELECT i.id, i.email, i.status, i.created_at, i.expires_at, i.token, r.name as role, r.id as role_id 
...
export const inviteEmployee = (req, res) => {
    ...
    db.get('SELECT id FROM employees WHERE email = ?', [email], (err, row) => {
        ...
        db.get("SELECT id FROM employee_invitations WHERE email = ? AND status = 'pending'", [email], (err, inviteRow) => {
            ...
            db.run(`INSERT INTO employee_invitations (id, email, role_id, token, status, expires_at)
         VALUES (?, ?, ?, ?, 'pending', ?)`, [invitationId, email, role_id, token, expiresAt], function (err) {
                if (err) {
...
export const deleteInvitation = (req, res) => {
    ...
    db.run(`DELETE FROM employee_invitations WHERE id = ?`, [invitationId], function (err) {
        if (err) {
...
export const createRole = (req, res) => {
    ...
    db.run(`INSERT INTO roles (name, description, is_system, permissions)
     VALUES (?, ?, 0, ?)`, [name, description || '', JSON.stringify(permissions || [])], function (err) {
        if (err) {
...
export const updateRole = (req, res) => {
    ...
    db.get(`SELECT * FROM roles WHERE id = ?`, [roleId], (err, role) => {
       ...
       WHERE id = ?`, [finalName, description || '', JSON.stringify(permissions || []), roleId], function (err) {
            if (err) {
...
export const deleteRole = (req, res) => {
    const roleId = req.params.id;
    db.get(`SELECT is_system FROM roles WHERE id = ?`, [roleId], (err, role) => {
        if (!role) {
            return res.status(404).json({ status: 'error', message: 'Role not found' });
        }
        if (role.is_system === 1) {
...
export const verifyInvitationToken = (req, res) => {
    const { token } = req.query;
    if (!token) {
        return res.status(400).json({ status: 'error', message: 'Token is required' });
    }
    db.get(`SELECT i.*, r.name as role_name 
     FROM employee_invitations i 
     JOIN roles r ON i.role_id = r.id 
     WHERE i.token = ?`, [token], (err, row) => {
        if (err) {
            return res.status(500).json({ status: 'error', message: 'Database error' });
        }
        if (!row) {
...
export const registerInvitedEmployee = (req, res) => {
    ...
    db.get(`SELECT i.*, r.name as role_name 
     FROM employee_invitations i 
     JOIN roles r ON i.role_id = r.id 
     WHERE i.token = ?`, [token], (err, invite) => {
        if (err || !invite) {
        ...
        bcrypt.hash(password, 10, (err, hash) => {
            ...
            db.serialize(() => {
                // Insert employee
                db.run(`INSERT INTO employees (id, role_id, first_name, last_name, email, password_hash, status, department)
             VALUES (?, ?, ?, ?, ?, ?, 'active', 'Operations')`, [employeeId, invite.role_id, first_name, last_name, invite.email, hash], (err) => {
                    if (err) {
                    ...
                    db.run(`UPDATE employee_invitations SET status = 'accepted' WHERE id = ?`, [invite.id], (err) => {
                        if (err)
