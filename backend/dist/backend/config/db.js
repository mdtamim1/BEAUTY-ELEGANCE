...
function initializeDatabase() {
    ...
    `);
            ...
            db.get("SELECT id FROM roles WHERE name = ?", [r.name], (err, row) => {
                const afterRoleProcessed = () => {
                    ...
                    if (processedCount === defaultRoles.length) {
                        ...
                        db.get("SELECT id FROM roles WHERE name = 'Super Admin'", (err, roleRow) => {
                            if (roleRow) {
                ...
                if (!row) {
                    db.run("INSERT INTO roles (name, description, is_system, permissions) VALUES (?, ?, ?, ?)", [r.name, r.desc, r.is_system, JSON.stringify(r.permissions)], afterRoleProcessed);
                }
                else {
                    db.run("UPDATE roles SET permissions = ?, description = ? WHERE id = ?", [JSON.stringify(r.permissions), r.desc, row.id], afterRoleProcessed);
                }
            });
        });
        ...
        db.run("DELETE FROM products WHERE id LIKE 'PRD-00%'", (err) => {
            if (err)
                console.error('Error deleting default products:', err);
            db.run("DELETE FROM product_gallery WHERE product_id LIKE 'PRD-00%'", (err) => {
                if (err)
                    console.error('Error deleting default gallery:', err);
        ...
        db.get("SELECT COUNT(*) as count FROM coupons", (err, row) => {
            if (row && row.count === 0) {
                ...
                defaultCoupons.forEach(c => {
                    db.run("INSERT INTO coupons (code, type, value, expiry, status) VALUES (?, ?, ?, ?, ?)", [c.code, c.type, c.value, c.expiry, c.status]);
                });
            }
        });
