...
export const getOrders = (req, res) => {
    db.all(`SELECT * FROM orders ORDER BY created_at DESC`, [], (err, orderRows) => {
        if (err) {
            console.error(err);
    ...
    db.serialize(() => {
        ...
        delivery_charge, discount, paid_amount, subtotal
            ...
            if (productsList && Array.isArray(productsList)) {
                const stmt = db.prepare(`INSERT INTO order_items (order_id, product_name, color, size, code, quantity, price) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`);
