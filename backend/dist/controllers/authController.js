...
export const login = (req, res) => {
    ...
    db.get(`SELECT e.*, r.name as role_name, r.permissions as role_permissions 
     FROM employees e 
     JOIN roles r ON e.role_id = r.id 
     WHERE e.email = ?`, [email], (err, employee) => {
        if (err) {
        ...
        bcrypt.compare(password, employee.password_hash, (err, isMatch) => {
            ...
            db.run(`UPDATE employees SET last_login_at = CURRENT_TIMESTAMP, last_login_ip = ? WHERE id = ?`, [lastLoginIp, employee.id]);
            res.json({
                status: 'success',
